  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Hatch Chart</h1>
      ...
      {loading ? (
        <div className="mt-8 space-y-3">
          {[1, 2, 3].map((i) => <div key={i} className="h-16 animate-pulse rounded-md bg-gray-100" />)}
        </div>
      ) : entries.length === 0 ? (
              ...
              {entries.map((entry) => (
                <tr key={entry.id} className="hover:bg-gray-50">
                  <td className="px-3 py-2.5 font-medium text-gray-900">
                    {entry.waterBody}
                    {entry.state && <span className="ml-1 text-xs text-gray-400">{entry.state}</span>}
                  </td>
                  <td className="px-3 py-2.5 text-gray-600">{MONTHS[entry.month - 1]}</td>
                  <td className="px-3 py-2.5 text-gray-600">{entry.species}</td>
                  <td className="px-3 py-2.5 font-medium text-gray-900">{entry.insectName}</td>
                  <td className="px-3 py-2.5 capitalize text-gray-500">{entry.insectType}</td>
                  <td className="px-3 py-2.5">
                    {entry.flyPattern ? (
                      ...
                      <span className="text-gray-600">{entry.patternName}</span>
                    )}
                  </td>
                  <td className="px-3 py-2.5 text-gray-500">{entry.timeOfDay ?? "—"}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
function HatchForm({ onSuccess }: { onSuccess: () => void }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-6 rounded-lg border border-gray-200 p-4">
      <h3 className="mb-3 text-sm font-semibold text-gray-900">Report a Hatch</h3>
      {error && <p className="mb-3 text-sm text-red-600">{error}</p>}

      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
        <input name="waterBody" required placeholder="Water body *" className="rounded-md border border-gray-300 px-3 py-1.5 text-sm" />
        <input name="region" required placeholder="Region *" className="rounded-md border border-gray-300 px-3 py-1.5 text-sm" />
        <input name="state" placeholder="State (optional)" className="rounded-md border border-gray-300 px-3 py-1.5 text-sm" />
        <select name="month" required defaultValue={new Date().getMonth() + 1} className="rounded-md border border-gray-300 px-3 py-1.5 text-sm">
          {MONTHS.map((m, i) => <option key={i} value={i + 1}>{m}</option>)}
        </select>
        <input name="species" required placeholder="Species *" className="rounded-md border border-gray-300 px-3 py-1.5 text-sm" />
        <input name="insectName" required placeholder="Hatch name *" className="rounded-md border border-gray-300 px-3 py-1.5 text-sm" />
        <select name="insectType" required className="rounded-md border border-gray-300 px-3 py-1.5 text-sm">
        ...
        <input name="patternName" required placeholder="Suggested pattern *" className="rounded-md border border-gray-300 px-3 py-1.5 text-sm" />
        <select name="timeOfDay" className="rounded-md border border-gray-300 px-3 py-1.5 text-sm">
      ...
      <textarea name="notes" placeholder="Notes (optional)" rows={2} className="mt-3 w-full rounded-md border border-gray-300 px-3 py-1.5 text-sm" />
